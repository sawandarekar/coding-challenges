...
public class EmasSuperComputer {
	...
	public ValidPlus(ValidPlus anotherValidPlus) {
	    area = anotherValidPlus.area;
	    length = anotherValidPlus.length;
	    cells = new ArrayList<Cell>(anotherValidPlus.cells);
	}
	...
	for (int i = 0; i < n; i++) {
		...
		if (currentCell == 'G') {
			...
			if (i - length >= 0 && map[i - length][j] == 'G' && i + length < n && map[i + length][j] == 'G'
				&& j - length >= 0 && map[i][j - length] == 'G' && j + length < m
				&& map[i][j + length] == 'G') {
